...
const zar = (n: number) => n.toLocaleString("en-ZA", { style: "currency", currency: "ZAR", maximumFractionDigits: 0 });

function stockBadge(stock: number, minStock: number | null) {
  ...
  if (stock <= threshold) return <span className="rounded-full bg-gold/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-gold">Low</span>;
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-ivory/60 text-left text-xs uppercase tracking-wider text-muted-foreground">
              ...
              <th className="px-5 py-3.5 font-semibold">Selling</th>
          ...
          <tbody className="divide-y">
            ...
            {products.map((p) => {
              const margin = p.costPrice != null ? Math.round(((p.price - p.costPrice) / p.price) * 100) : null;
              return (
                <tr key={p.id} className="transition hover:bg-ivory/40">
                  ...
                  <td className="px-5 py-3.5">{zar(p.price)}</td>
                  <td className="px-5 py-3.5 text-muted-foreground">{p.costPrice != null ? zar(p.costPrice) : "—"}</td>
                  <td className="px-5 py-3.5">
                    {margin != null ? (
                      <span className={cn("font-semibold", margin >= 40 ? "text-emerald-600" : margin >= 20 ? "text-gold" : "text-destructive")}>
                        {margin}%
                      </span>
                    ) : (
                  ...
                  <td className="px-5 py-3.5">
                    ...
                    <span className="ml-1 text-xs text-muted-foreground">/ {p.minStock ?? 5} min</span>
                  </td>
                  <td className="px-5 py-3.5">{stockBadge(p.stock, p.minStock)}</td>
